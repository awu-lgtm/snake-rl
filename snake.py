    ...
    def draw_snake(self, display, state: 'State'):
        ...
        for i, pt in enumerate(self.snake):
            if i == 0:
                match self.dir:
                    case Direction.RIGHT:
                        display.blit(snake_head_r, (pt.x * BLOCK_SIZE, pt.y * BLOCK_SIZE))
                        pygame.draw.rect(display, SHADOW_GREEN, pygame.Rect(pt.x * BLOCK_SIZE, pt.y * BLOCK_SIZE + BLOCK_SIZE, 1.49 * BLOCK_SIZE, SHADOW_SIZE))
                    case Direction.LEFT:
                        display.blit(snake_head_l, (pt.x * BLOCK_SIZE - 0.47 * BLOCK_SIZE, pt.y * BLOCK_SIZE))
                    ...
                    case Direction.UP:
                        display.blit(snake_head_u, (pt.x * BLOCK_SIZE, pt.y * BLOCK_SIZE - 0.25 * BLOCK_SIZE))
                    case Direction.DOWN:
                        display.blit(snake_head_d, (pt.x * BLOCK_SIZE, pt.y * BLOCK_SIZE))
                        pygame.draw.rect(display, SHADOW_GREEN, pygame.Rect(pt.x * BLOCK_SIZE, pt.y * BLOCK_SIZE + 1.25 * BLOCK_SIZE, BLOCK_SIZE, SHADOW_SIZE))
            # if i % 2 != 0:
...
class State:
    ...
    def is_collision(self, point: Point):
        x,y = point.x, point.y
        return self.is_out_of_bounds(point) or self.state[y, x] == Square.SNAKE
...
class SnakeEnv(SnakeGame, gym.Env):
    ...
    def step(self, action):
        ...
        next_obs = self.game_state.get_state_copy()
        info = dict()
        return next_obs, done, reward, info
    
    def reset(self):
        self.game_state.reset()
        obs = self.game_state.get_state_copy()
        return obs
